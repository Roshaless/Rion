...
public static class RFile
{
    ...
    public static RStringTable Read(string path)
    {
        ...
        {
            return new RStringTable(reader.Metadata, reader.ReadAll());
        }
    }
    ...
    public static RStringTable Read(ReadOnlySpan<byte> data)
    {
        ...
        {
            return new RStringTable(reader.Metadata, reader.ReadAll().ToImmutableArray());
        }
    }

    /// <summary>
    /// Reads an <see cref="RStringTable.RecordRStringTable"/> from the specified file path.
    /// </summary>
    /// <param name="path">The path to the file.</param>
    /// <returns>An <see cref="RStringTable.RecordRStringTable"/> instance.</returns>
    public static RStringTable.RecordRStringTable ReadAsRecord(string path)
    {
        using var scope = RFileBufferScope.CreateFrom(path);
        var reader = new RStringTableReader(scope.Span);
        {
            return new RStringTable.RecordRStringTable(reader.Metadata, reader.ReadAll().ToImmutableArray());
        }
    }

    /// <summary>
    /// Reads an <see cref="RStringTable.RecordRStringTable"/> from the specified data.
    /// </summary>
    /// <param name="data">The data to read from.</param>
    /// <returns>An <see cref="RStringTable.RecordRStringTable"/> instance.</returns>
    public static RStringTable.RecordRStringTable ReadAsRecord(ReadOnlySpan<byte> data)
    {
        var reader = new RStringTableReader(data);
        {
            return new RStringTable.RecordRStringTable(reader.Metadata, reader.ReadAll());
        }
    }
