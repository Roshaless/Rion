...
/// <typeparam name="TInput">The type of the input string table.</typeparam>
public abstract class RStringTableConverter<TConvert, TInput>() :
    RStringTableConverter(typeof(TConvert), typeof(TInput))
    where TConvert : IRStringTable where TInput : IRStringTable
{
    /// <summary>
    ...
    public abstract void Write(Stream stream, TInput value);
    ...
    /// <param name="stream">The stream to write to.</param>
    /// <param name="value">The string table to write.</param>
    internal override void WriteCore(Stream stream, object value) => Write(stream, (TInput)value);
}
