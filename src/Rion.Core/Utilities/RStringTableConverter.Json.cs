...
public partial class RStringTableConverter
{
    ...
    private sealed class JsonConverterImpl : RStringTableConverter<RStringTable, IRStringTable>
    {
        ...
        /// <inheritdoc />
        public override RStringTable? Read(ReadOnlySpan<byte> bytes)
        {
            // Read the JSON.
        ...
        public override void Write(Stream stream, IRStringTable value)
        {
            ...
            writer.WriteStartObject();
            {
                ...
                if (value.Metadata is ILegacyFontConfigMetadata { FontConfig: not null } legacyMetadata)
                {
                ...
                writer.WriteStartObject();
                {
                    foreach (var pair in value.OrderBy(item => RSTHashtable.GetNameOrHexString(item.Key)))
                    {
