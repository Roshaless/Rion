...
/// </summary>
public abstract class StringTableFileReaderProvider
{
    /// <summary>
    /// Gets the provider for the latest supported string table.
    /// </summary>
    /// <remarks>Use this property to access the most recent version of the <see cref="StringTableFileReaderProvider"/>.
    /// This is typically recommended unless a specific older version is required for compatibility.</remarks>
    public static StringTableFileReaderProvider Default => PatchV1502;

    /// <summary>
    /// Gets the provider for reading string tables of version 15.2.
    /// </summary>
    public static StringTableFileReaderProvider PatchV1502 { get; } = new PatchV1502ReaderProvider();

    /// <summary>
    /// Gets the provider for reading string tables of version 14.15.
    /// </summary>
    public static StringTableFileReaderProvider PatchV1415 { get; } = new PatchV1415ReaderProvider();
    ...
    public static StringTableFileReaderProvider LegacyV5 { get; } = new LegacyV5ReaderProvider();

    /// <summary>
    /// Abstract method to read and extract the properties of a string table from the provided binary data reader.
    /// Implementations must parse the binary data to populate an instance of <see cref="StringTableFileProperties"/>.
    /// </summary>
    /// <param name="reader">An instance of <see cref="RawMemoryReader"/> used to read the string table properties from binary data.</param>
    /// <returns>A <see cref="StringTableFileProperties"/> object containing the extracted string table properties.</returns>
    public abstract StringTableFileProperties ReadFileProperties(RawMemoryReader reader);
    ...
    /// This class is a concrete subclass of <see cref="StringTableFileReaderProvider" />, offering standard behavior
    /// for interpreting string table files, property extraction, and string decoding.
    /// </summary>
    private abstract class V5PatchReaderProvider : StringTableFileReaderProvider
    {
        /// <inheritdoc />
        public override StringTableFileProperties ReadFileProperties(RawMemoryReader reader)
        {
            var version = reader.Read<int>() >> 24;
            var metadata = version switch
            {
                5 => GetV5Metadata(),
                4 => StringTableMetadata.Version4,
                3 => StringTableMetadata.Version3,
            ...
            return new StringTableFileProperties(metadata, entryCount, hashesOffset, contentOffset);
        }

        /// <inheritdoc />
        public override string ReadString(ReadOnlySpan<byte> span) => StringRefManager.GetOrAdd(span);

        /// <summary>
        /// Retrieves the metadata associated with the version 5 string table.
        /// </summary>
        /// <returns>An <see cref="IStringTableMetadata"/> instance containing metadata for the version 5 string table.</returns>
        protected abstract IStringTableMetadata GetV5Metadata();
        ...
        /// </summary>
        /// <param name="reader">
        /// The binary reader (<see cref="RawMemoryReader" />) positioned at the beginning of the legacy
        /// metadata section.
        /// </param>
        /// <returns>
        ...
        /// </returns>
        private static LegacyFontConfigMetadata ReadLegacyFontConfigMetadata(RawMemoryReader reader)
        {
            // For Old Version 2
            var hasConfig = reader.Read<byte>();
            return new LegacyFontConfigMetadata(hasConfig is not 0
                ? StringRefManager.GetOrAdd(reader.Read(reader.Read<int>()))
                : null);
        }
    }
    ...
    {
        /// <inheritdoc />
        protected override IStringTableMetadata GetV5Metadata() => StringTableMetadata.Version5Patch1502;
    }
    ...
    {
        /// <inheritdoc />
        protected override IStringTableMetadata GetV5Metadata() => StringTableMetadata.Version5Patch1415;
    }
    ...
    {
        /// <inheritdoc />
        protected override IStringTableMetadata GetV5Metadata() => StringTableMetadata.Version5Legacy;
    }
}
